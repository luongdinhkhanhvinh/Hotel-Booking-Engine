<form class="bravo-form-login" method="POST" action="{{ route('login') }}">
    ...
    <div class="form-group">
        <input type="text" class="form-control" name="email" autocomplete="off" placeholder="{{__('Địa chỉ e-mail')}}">
        <i class="input-icon fa fa-envelope-o"></i>
        <span class="invalid-feedback error error-email"></span>
    </div>
    <div class="form-group">
        <input type="password" class="form-control" name="password" autocomplete="off"  placeholder="{{__('Mật khẩu')}}">
    ...
    <div class="form-group">
        <div class="d-flex justify-content-between">
            <label for="remember-me" class="mb0">
                <input type="checkbox" name="remember" id="remember-me" value="1"> {{__('Ghi nhớ')}} <span class="checkmark fcheckbox"></span>
            </label>
            <a href="{{ url("/password/reset") }}">{{__('Quên mật khẩu?')}}</a>
        </div>
    </div>
    ...
    @endif
    <div class="c-grey font-medium f14 text-center"> {{__('Bạn chưa có tài khoản?')}} <a href="" data-target="#register" data-toggle="modal">{{__('Đăng ký')}}</a>
    </div>
</form>