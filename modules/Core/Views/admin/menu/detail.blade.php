    ...
    <div class="container-fluid" id="menu-app" data-id="{{$row->id ?? ''}}" v-cloak>
        ...
        <div class="row">
            <div class="col-md-4 col-xl-3 menu-item-types">
                <div v-for="(type,index) in item_types" class="panel panel-toggle-able">
                    ...
                    <div class="panel-body" v-show="type.open">
                        <input type="text" placeholder="{{__('Tìm kiếm...')}}" class="form-control input-sm menu-search"
                        ...
                        <div class="alert-text danger mt10" v-show="!type.items.length">{{__("Không có mục nào được tìm thấy")}}</div>
                        <div class="text-right">
                            <span class="btn btn-sm btn-primary" @click="addToMenu(type)">{{__('Thêm vào bảng chọn')}}</span>
                        </div>
                    </div>
                </div>
                <div class="panel panel-toggle-able">
                    <div class="panel-title" @click="custom_show = custom_show ? false : true">{{__('Điều chỉnh Url')}}
                        <i class="icon ion-md-arrow-dropdown"></i>
                    </div>
                    <div class="panel-body" v-show="custom_show">
                        ...
                        <div class="form-group">
                            <label>{{__('Liên kết văn bảng')}}</label>
                            <input type="text" v-model="custom_name" class="form-control input-sm">
                        </div>
                        <div class="text-right">
                            <span class="btn btn-sm btn-primary" @click="addCustomUrl">{{__('Thêm vào bảng chọn')}}</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-8 col-xl-9">
                <div class="panel">
                    <div class="panel-title">{{__('Bảng chọn mục')}}</div>
                    <div class="panel-body">
                                ...
                                <div slot-scope="{data,store}" class="nestable-item-content" v-if="!data.isDragPlaceHolder">
                                    ...
                                    <div class="menu-info" v-show="data._open">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>{{__('Mục tiêu')}}</label>
                                                    <select v-model="data.target" class="input-sm form-control">
                                                        <option value="">{{__('Bình thường')}}</option>
                                                        <option value="_blank">{{__('Mở cửa sổ mới')}}</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="d-flex justify-content-between">
                                            <a href='#' @click="deleteMenuItem($event,data,store)"
                                               class="alert-text danger delete-menu-item">{{__('Xóa')}}</a>
                                            <span v-show="data.origin_name">{{__('Gốc: ')}} <a
                        ...
                        <br>
                        <br>
                        <h3 class="panel-body-title">{{__('Cấu hình bảng chọn')}}</h3>
                    ...
                    <div class="panel-footer text-right">
                        ...
                        <span class="btn btn-success" @click="saveMenu">{{__("Lưu bảng chọn")}}</span>
                    </div>
                </div>
            </div>
        </div>
