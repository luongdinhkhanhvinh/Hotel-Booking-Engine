...
class BookingController extends \App\Http\Controllers\Controller
{
    ...
    public function doCheckout(Request $request)
    {
        ...
        $service = $booking->service;
        if (empty($service)) {
            $this->sendError(__("Không tìm thấy dịch vụ"));
        }
        /**
         * Google ReCapcha
         */
        if(ReCaptchaEngine::isEnable() and setting_item("booking_enable_recaptcha")){
            ...
            if(!$codeCapcha or !ReCaptchaEngine::verify($codeCapcha)){
                $this->sendError(__("Vui lòng xác thực bằng captcha"));
            }
        }
        $rules = [
        ...
        if (!empty($rules['payment_gateway'])) {
            ...
            if (empty($gateways[$payment_gateway]) or !class_exists($gateways[$payment_gateway])) {
                $this->sendError(__("Không tìm thấy cổng thanh toán"));
            }
            $gatewayObj = new $gateways[$payment_gateway]($payment_gateway);
            if (!$gatewayObj->isAvailable()) {
                $this->sendError(__("Cổng thanh toán không khả dụng"));
            }
        }
    ...
    public function addToCart(Request $request)
    {
        ...
        if (empty($allServices[$service_type])) {
            $this->sendError(__('Không tìm thấy loại dịch vụ'));
        }
        ...
        if (!$service->isBookable()) {
            $this->sendError(__('Dịch vụ không thể đặt trước'));
        }
        //        try{
    ...
    public function detail(Request $request, $code)
    {
        ...
        $data = [
            'page_title' => __('Chi tiết đặt phòng'),
            'booking'    => $booking,
            'service'    => $booking->service,
        ];
        if ($booking->gateway) {
