...
@section('content')
    <div class="container-fluid">
        <div class="dashboard-page">
            <h4 class="welcome-title text-uppercase">{{__('Chào mừng :name!',['name'=>Auth::user()->nameOrEmail])}}</h4>
        </div>
        <br>
        <div class="row">
            @if(!empty($top_cards))
            ...
            <div class="col-md-12 col-lg-6 mb-3">
                <div class="panel">
                    <div class="panel-title d-flex justify-content-between align-items-center">
                        <strong>{{__('Thống kê thu nhập')}}</strong>
            ...
            <div class="col-md-12 col-lg-6 ">
                <div class="panel">
                    <div class="panel-title d-flex justify-content-between">
                        <strong>{{__('Đặt phòng gần đây')}}</strong>
                        <a href="{{url('admin/module/report/booking')}}" class="btn-link">{{__("Nhiều hơn")}}
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tr>
                                    <th width="60px">#</th>
                                    <th>{{__('Mục')}}</th>
                                    <th width="100px">{{__("Toàn bộ")}}</th>
                                    <th width="100px">{{__("Trạng thái")}}</th>
                                    <th width="100px">{{__("Tạo bởi")}}</th>
                                </tr>
                                </thead>
                                <tbody>
...
@section('script.body')
    ...
    <script>
            ...
            options: {
                ...
                scales: {
                    ...
                    yAxes: [{
                        ...
                        scaleLabel: {
                            display: true,
                            labelString: '{{__("Đơn vị tiền tệ: :currency_main",['currency_main'=>setting_item('currency_main')])}}'
                        }
                    }]
                },
                tooltips: {
        ...
        $('#reportrange').daterangepicker({
            ...
            "showDropdowns": true,
            ranges: {
                '{{__("Hôm nay")}}': [moment(), moment()],
                '{{__("Hôm qua")}}': [moment().subtract(1, 'days'), moment().subtract(1, 'days')],
                '{{__("7 ngày qua")}}': [moment().subtract(6, 'days'), moment()],
                '{{__("30 ngày qua")}}': [moment().subtract(29, 'days'), moment()],
                '{{__("Tháng này")}}': [moment().startOf('month'), moment().endOf('month')],
                '{{__("Tháng qua")}}': [moment().subtract(1, 'month').startOf('month'), moment().subtract(1, 'month').endOf('month')],
                '{{__("Năm này")}}': [moment().startOf('year'), moment().endOf('year')],
                '{{__('Tuần này')}}': [moment().startOf('week'), end]
            }
