...
class Tour extends Bookable
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $this->sendError(__("Không thể kiểm tra phòng trống"));
    }
        ...
        if ($meta) {
            ...
            if ($meta->enable_open_hours) {
                ...
                if (!isset($open_hours[$nDate]) or empty($open_hours[$nDate]['enable'])) {
                    $this->sendError(__("Chuyến đi không thể mở vào ngày bạn chọn"));
                }
            }
        }
        return true;
        ...
        if ($meta) {
            ...
            if ($meta->enable_extra_price) {
                ...
                foreach ($booking_data['extra_price'] as $k => &$type) {
                    ...
                    switch ($type['type']) {
                        case "per_day":
                            $type['price_type'] .= '/' . __('ngày');
                            break;
                        case "per_hour":
                            $type['price_type'] .= '/' . __('giờ');
                            break;
                    }
                    if (!empty($type['per_person'])) {
                        $type['price_type'] .= '/' . __('khách');
                    }
                }
            }
            if ($meta->enable_open_hours) {
    ...
    public function getReviewDataAttribute()
    {
        $list_score = [
            'score_total'  => 0,
            'score_text'   => __("Không có đánh giá"),
            'total_review' => 0,
            'rate_score'   => [],
        ];
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number chuyến đi", ['number' => $number]);
        }
        return __(":number chuyến đi", ['number' => $number]);
    }
