...
class TourController extends AdminController
{
    ...
    public function create(Request $request)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                ...
                $row->saveSEO($request);
                return redirect('admin/module/tour')->with('success', __('Tạo chuyến đi!'));
            }
        } else {
            $row = new Tour();
            $row->fill([
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                ...
                $row->saveMeta($request);
                $row->saveSEO($request);
                return redirect('admin/module/tour/edit/'.$id)->with('success', __('Cập nhật chuyến đi!'));
            }
        }
        $data = [
            'row'            => $row,
    ...
    public function bulkEdit(Request $request)
    {
        ...
        if (empty($ids) or !is_array($ids)) {
            return redirect()->back()->with('error', __('Không có mục nào được chọn!'));
        }
        if (empty($action)) {
            return redirect()->back()->with('error', __('Vui lòng chọn một hành động!'));
        }
        if ($action == "delete") {
            foreach ($ids as $id) {
