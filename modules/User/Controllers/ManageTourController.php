...
class ManageTourController extends FrontendController
{
    ...
    public function createTour(Request $request)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                ...
                $row->saveSEO($request);
                return redirect('user/tour')->with('success', __('Tạo chuyến đi'));
            }
        } else {
            $row = new Tour();
            $row->fill([
    ...
    public function editTour(Request $request, $id)
    {
        ...
        $row = $row->find($id);
        if (empty($row)) {
            return redirect('user/tour')->with('warning', __('Không tìm thấy chuyến đi!'));
        }
        if (!empty($request->input())) {
            $request->validate([
                'title' => 'required',
            ...
            if ($row->save()) {
                $row->saveMeta($request);
                $row->saveSEO($request);
                return redirect('user/tour')->with('success', __('Cập nhật chuyến đi!'));
            }
        }
        $data = [
            'row'           => $row,
    ...
    public function deleteTour($id)
    {
        ...
        return redirect('user/tour')->with('success', __('Chuyến đi xóa thành công!'));
    }
}