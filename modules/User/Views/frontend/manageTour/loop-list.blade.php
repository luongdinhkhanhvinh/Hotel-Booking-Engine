        ...
        <div class="col-md-4">
            ...
            <div class="control-action">
                ...
                @if(Auth::user()->hasPermissionTo('tour_update'))
                    <a href="{{url("user/tour/edit/".$row->id)}}" class="btn btn-warning">{{__("Sửa")}}</a>
                @endif
                @if(Auth::user()->hasPermissionTo('tour_delete'))
                    <a href="{{url("user/tour/del/".$row->id)}}" class="btn btn-danger">{{__("Xóa")}}</a>
                @endif
            </div>
        </div>
    </div>
