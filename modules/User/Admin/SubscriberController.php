...
class SubscriberController extends AdminController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data = [
            ...
            'breadcrumbs' => [
                ...
                [
                    'name'  => __('Sửa: :email', ['email' => $row->email]),
                    'class' => 'active'
                ],
            ]
        ];
    ...
    public function store(Request $request)
    {
        ...
        if ($check and $check->id != $request->input('id')) {
            return redirect()->back()->with('error', __('Email tồn tại'));
        }
        $row->fill($request->input());
        if ($row->save()) {
            return redirect()->back()->with('success', __('Cập nhật người đăng ký'));
        }
    }
    ...
    public function editBulk(Request $request)
    {
        ...
        if (empty($ids) or !is_array($ids)) {
            return redirect()->back()->with('error', __('Chọn ít nhất nhất một mục!'));
        }
        if (empty($action)) {
