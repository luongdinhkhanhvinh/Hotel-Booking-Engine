...
class UserController extends AdminController
{
    ...
    public function changepass(Request $request, $id)
    {
        ...
        if ($password != $password_confirmation) {
            return redirect()->back()->with("error", __("Mật khẩu mới của bạn nhập không khớp.Vui lòng nhập lại !!"));
        }
        if ($urow->id != Auth::user()->id and !Auth::user()->hasPermissionTo('user_update')) {
            if ($password) {
            ...
            if ($password) {
                if (!(Hash::check($request->input('old_password'), $urow->password))) {
                    ...
                    return redirect()->back()->with("error", __("Mật khẩu hiện tại của bạn không khớp với mật khẩu bạn cung cấp."));
                }
            }
        }
        ...
        if ($urow->save()) {
            ...
            return redirect()->back()->with('success', __('Cập nhật mật khẩu!'));
        }
    }
    ...
    public function store(Request $request, $id)
    {
        ...
        if ($row->save()) {
            ...
            return redirect('admin/module/user')->with('success', ($id and $id>0) ? __('Cập nhật người dùng'):__("Tạo người dùng"));
        }
    }
    ...
    public function bulkEdit(Request $request)
    {
        ...
        if (empty($ids))
            return redirect()->back()->with('error', __('Chọn ít nhất một mục!'));
        if (empty($action))
            return redirect()->back()->with('error', __('Chọn một hành động!'));
        if ($action == 'delete') {
            foreach ($ids as $id) {
