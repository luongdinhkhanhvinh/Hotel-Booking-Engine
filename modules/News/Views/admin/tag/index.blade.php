...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{ __('Thẻ tin tức')}} </h1>
        </div>
        @include('admin.message')
        <div class="row">
            <div class="col-md-4">
                <div class="panel">
                    <div class="panel-title">{{ __('Thêm thẻ')}}</div>
                ...
                <div class="filter-div d-flex justify-content-between ">
                    ...
                    <div class="col-left">
                        <form method="get" action="{{url('/admin/module/news/tag/')}} " class="filter-form filter-form-right d-flex justify-content-end" role="search">
                            @csrf
                            <input placeholder="{{__("Tìm từ khóa ...")}}" type="text" name="s" value="{{ Request()->s }}" class="form-control">
                            <button class="btn-info btn btn-icon btn_search" id="search-submit" type="submit">{{__('Tìm kiếm thẻ')}}</button>
                        </form>
                    </div>
                </div>
