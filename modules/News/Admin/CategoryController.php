...
class CategoryController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($request->input('name'))) {
            ...
            if (NewsCategory::where('name', $request->input('name'))->where('parent_id',$request->input('parent_id'))->exists()) {
                return redirect()->back()->with('error', __('Danh mục không tồn tại !'));
            } else {
                $row->status = 'publish';
                $row->parent_id = $request->input('parent_id');
                if ($row->save()) {
                    $row->saveSEO($request);
                    return redirect('admin/module/news/category')->with('success', __(' Danh mục được tạo!'));
                }
            }
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                $row->saveSEO($request);
                return redirect('admin/module/news/category')->with('success', __('Danh mục được cập nhật'));
            }
        }
        $data = [
            'row'     => $row,
    ...
    public function bulkEdit(Request $request)
    {
        ...
        if (empty($ids) or !is_array($ids)) {
            return redirect()->back()->with('error', __('Vui lòng chọn ít nhất một sản phẩm!'));
        }
        if (empty($action)) {
