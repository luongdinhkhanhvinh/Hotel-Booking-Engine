...
class LanguageController extends AdminController
{
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post') and !empty($request->input())) {
            ...
            return redirect('admin/module/language')->with('success', __("Ngôn ngữ được tạo"));
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (!empty($request->input())) {
            ...
            if ($row->save()) {
                return redirect()->back()->with('success', __('Ngôn ngữ được cập nhật'));
            }
        }
        $data = [
            'row'         => $row,
            ...
            'breadcrumbs' => [
                ...
                [
                    'name'  => __('Sửa: :name', ['name' => $row->name]),
                    'class' => 'active'
                ],
            ]
        ];
    ...
    public function editBulk(Request $request)
    {
        ...
        if (empty($action)) {
            return redirect()->back()->with('error', __('Chọn một hoạt động!'));
        }
        if ($action == "delete") {
            foreach ($ids as $id) {
