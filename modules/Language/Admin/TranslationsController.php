...
class TranslationsController extends AdminController
{
    ...
    public function detail(Request $request, $id)
    {
        ...
        $data = [
            ...
            'breadcrumbs' => [
                [
                    'name' => __('Quản lý dịch'),
                    'url'  => 'admin/module/language/translations'
                ],
                [
                    'name'  => __('Dịch cho: :name', ['name' => $lang->name]),
                    'class' => 'active'
                ],
            ]
        ];
    ...
    public function store(Request $request, $id)
    {
        ...
        return redirect()->back()->with('success', __("Bản dịch được lưu"));
    }
    ...
    public function build($id)
    {
        ...
        if (!is_writable(base_path('resources/lang'))) {
            return redirect($back)->with('error', __("Thư mục: resources/lang không thể ghi. Vui lòng liên hệ với nhà cung cấp dịch vụ lưu trữ của bạn"));
        }
        if (file_exists($file) and !is_writable($file)) {
            return redirect($back)->with('error', __("Tập tin: :file_name không thể viết được. Vui lòng liên hệ với nhà cung cấp dịch vụ lưu trữ của bạn", ['file_name' => 'resources/lang/' . $lang->locale . '.json']));
        }
        $query = Translation::select([
            'core_translations.*',
            't.string as origin'
        ...
        $lang->save();
        return redirect(url('admin/module/language/translations'))->with('success', __("Xây dựng lại tập tin ngôn ngữ cho: :name hoàn thành", ['name' => $lang->name]));
    }
    ...
    public function loadStrings(){
        ...
        if(!is_file($file)){
            return redirect($back)->with('error', __("Nguồn ngôn ngữ mặc định không tồn tại"));
        }

        $content = file_get_contents($file);
        if(empty($content)){
            return redirect($back)->with('error', __(" Nguồn ngôn ngữ mặc định trống"));
        }

        $json = \GuzzleHttp\json_decode($content,true);
        if(empty($json)){
            return redirect($back)->with('error', __("Nguồn ngôn ngữ mặc định không có bất kỳ chuỗi nào"));
        }
        ...
        return redirect($back)->with('success', __("Nạp vào :count ký tự",['count'=>count($json)]));
    }
    public function genDefault(){
        ...
        return redirect($back)->with('success', __("Tạo ngôn ngữ JSON mặc định"));
    }
