...
class MediaController extends AdminController
{
    ...
    public function store(Request $request)
    {
        ...
        if (!$this->hasPermissionMedia()) {
            $this->sendError('Không có quyền tải lên');
        }
        $fileName = 'file';
        ...
        if (empty($file)) {
            $this->sendError(__("Vui lòng chọn tập tin"));
        }
        try {
        ...
        $this->sendError(__("Không thể lưu trữ tiệp"));
    }
    ...
    public static function validateFile($file, $group = "default")
    {
        ...
        if (!in_array(strtolower($file->getClientOriginalExtension()), $config['types'])) {
            throw new \Exception(__("Loại tiệp không được phép"));
        }
        if ($file->getSize() > $config['max_size']) {
            throw new \Exception(__("Kích thước tiệp tải lên tối đa là :max_size B", ['max_size' => $config['max_size']]));
        }
        return true;
    }
