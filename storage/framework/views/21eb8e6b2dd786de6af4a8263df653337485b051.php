        ...
        <div class="modal-content">
            <div id="cdn-browser" class="cdn-browser d-flex flex-column" v-cloak>
                    ...
                    <div class="d-flex justify-content-between">
                        <div class="col-left d-flex align-items-center">
                            <div class="filter-item">
                                <input type="text" placeholder="Tìm kiêm tên tập tin...." class="form-control" v-model="filter.s" @keyup.enter="filter.page = 1;reloadLists()">
                            </div>
                            <div class="filter-item">
                                <button class="btn btn-default" @click="filter.page = 1;reloadLists()">
                                    <i class="fa fa-search"></i> <?php echo e(__("Tìm kiếm")); ?></button>
                            </div>
                            <div class="filter-item">
                                <small><i><?php echo e(__("Tổng cộng")); ?>: {{total}} <?php echo e(__("tập tin")); ?></i></small>
                            </div>
                        </div>
                        <div class="col-right">
                            <i class="fa-spin fa fa-spinner icon-loading"></i>
                            <button class="btn btn-success btn-pick-files">
                                <span><i class="fa fa-upload"></i> <?php echo e(__("Tải lên")); ?></span>
                ...
                <div class="files-list">
                    ...
                    <p class="no-files-text text-center" v-show="!total && apiFinished" style="display: none"><?php echo e(__("Không tìm thấy tập tin nào")); ?></p>
                    <div class="text-center" v-if="totalPage > 1">
                        <nav aria-label="Page navigation example">
                            <ul class="pagination">
                                <li class="page-item" :class="{disabled:filter.page <= 1}">
                                    <a class="page-link" v-if="filter.page <=1"><?php echo e(__("Trước")); ?></a>
                                    <a class="page-link" href="#" v-if="filter.page > 1" v-on:click="changePage(filter.page-1,$event)"><?php echo e(__("Trước")); ?></a>
                                </li>
                                ...
                                <li class="page-item" :class="{disabled:filter.page >= totalPage}">
                                    <a v-if="filter.page >= totalPage" class="page-link"><?php echo e(__("Tiếp")); ?></a>
                                    <a href="#" class="page-link" v-if="filter.page < totalPage" v-on:click="changePage(filter.page+1,$event)"><?php echo e(__("Tiếp")); ?></a>
                                </li>
                            </ul>
                        </nav>
                    </div>
                ...
                <div class="browser-actions d-flex justify-content-between flex-shrink-0" v-if="selected.length">
                    <div class="col-left" v-show="selected.length">
                        <div v-if="selected && selected.length">
                            <div class="count-selected">{{selected.length}} <?php echo e(__("Tập tin được chọn")); ?></div>
                            <div class="clear-selected" @click="selected=[]"><i><?php echo e(__("Bỏ chọn")); ?></i></div>
                        </div>
                    </div>
                    <div class="col-right" v-show="selected.length">
                        <button class="btn btn-primary" :class="{disabled:!selected.length}" @click="sendFiles"><?php echo e(__("Sử dụng tập tin")); ?></button>
                    </div>
                </div>
            </div>
        </div>
...
<?php /**PATH C:\laragon\www\Booking Core 1.1.0\booking-core\modules/Media/Views/browser.blade.php ENDPATH**/ ?>